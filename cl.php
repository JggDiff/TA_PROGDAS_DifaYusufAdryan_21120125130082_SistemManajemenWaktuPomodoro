 ...
 */
function addTaskToQueue($name, $sessions) {
    if (!empty(trim($name)) && (int)$sessions > 0) {
        $_SESSION['task_queue'][] = [
...
/**
 * Hapus tugas dari antrian berdasarkan indeks
 */
...
/**
 * Bangun jadwal Pomodoro dari antrian
 */
function buildPomodoroSchedule() {
    $schedule = [];
    $tasks = $_SESSION['task_queue'] ?? [];
    
    for ($ti = 0; $ti < count($tasks); $ti++) {
        $task = $tasks[$ti];
        $isLastTask = ($ti === count($tasks) - 1);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_task') {
    addTaskToQueue($_POST['task_name'] ?? '', $_POST['session_count'] ?? 0);
}


if (isset($_GET['delete'])) {
    removeTaskFromQueue(intval($_GET['delete']));
...
$schedule = buildPomodoroSchedule();
...
<head>
    <meta charset="UTF-8">
    <title>Pomodoro Planner (Dengan Fungsi)</title>
    <style>
        body {
...
<body>
    <div class="container">
        <h1>🍅 Pomodoro Planner (Pure PHP + Fungsi)</h1>
        ...
        <form method="POST">
            ...
            <div class="form-group">
                ...
                <input type="number" name="session_count" id="session_count" min="1" value="2" required>
            </div>
